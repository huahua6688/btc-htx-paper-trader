...
export function formatCycle(result) {
  const lines = [
    `V1 monitor 完成：${result.report.generatedAt}`,
...
export function formatStatus(db) {
  ...
  if (snapshot) lines.push(`最近决策：${snapshot.decision}，BTC ${n(snapshot.price, 1)} USDT，置信度 ${n(snapshot.confidence_pct, 0)}%`);
  if (position) {
...
export function formatReport(db) {
  ...
  return [
    "BTC/USDT V1 Paper Trading 绩效报告",
